  ...
  function draw() {
    ...
    if (statsSwitch) {
      ...
      for (var i = 0; i < colonies.length; i++) {
        ...
        context.fillText('colonyId ' + i + ': ' + colonies[i].workers.length, 0, 10 * i + 10);
      }
      context.fillStyle = 'blue';
      context.fillText('Total Worker Ants: ' + totalWorkers, 0, 10 * colonies.length + 10);
      context.fillText('Total Food: ' + totalFood, 0, 10 * (colonies.length + 1) + 10);
      context.fillText('Total ScentMapObjects: ' + environment.scentMap.length, 0, 10 * (colonies.length + 2) + 10);
    }
  }
}
